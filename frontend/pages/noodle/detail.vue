<template>
  <view class="city-page">
    ...
    <view class="section-title-row">
      ...
      <view class="must-eat-icon-wrapper" @tap="goMustEat">
        <text class="must-eat-hint">一键获取</text>
        <image :src="IMG.bichi" class="must-eat-icon" mode="aspectFit" />
      </view>
    </view>
      ...
      <view class="modal" @tap.stop>
        ...
        <view class="upload-form">
          ...
          <view class="location-buttons">
            <button class="location-btn" type="default" size="mini" @tap="chooseLocation">选择位置</button>
            <button class="location-btn" type="default" size="mini" @tap="locateWithAMap">用高德定位</button>
          </view>
    ...
    <view class="post-list">
      <view v-for="(it, idx) in posts" :key="it.id" class="post-card">
        ...
        <text class="post-caption">{{ it.caption || '' }}</text>
        <view class="post-meta">
          <view v-if="it.location" class="location-info">
            <text class="location-icon">📍</text>
            <text class="location-text">{{ it.location }}</text>
          </view>
          <view class="time-info">
            <text class="time-icon">🕐</text>
            <text class="time-text">{{ formatTime(it.createdAt || it.createTime || it.timestamp) }}</text>
          </view>
        </view>
        <text
          v-if="canDeletePost(it)"
          class="post-del"
          @tap.stop="removePost(idx)"
        >删除</text>
      </view>
    </view>
...
export default {
  ...
  computed: {
    cityHero() {
      ...
      const defaultHero = IMG.retouch_2025111814274381
  ...
  methods: {
    ...
    getCityNoodles(id) {
      // 保留静态数据作为后备（接口失败时兜底）
      const img = (slug) => IMG.retouch_2025112414392946
      const data = {
        xian: [
      ...
      api(cityName, tag)
        .then(res => {
          ...
          this.noodles = list.map(it => {
            ...
            const img = it.imageUrl || it.img || IMG.retouch_2025112414392946
            let tags = []
    ...
    async submitPost() {
      ...
      const hasImages = this.draftImages.length > 0

      // 允许：只发图片、只发文案、或图文都有；但不能完全空
      if (!caption && !hasImages) {
        uni.showToast({ title: '请至少填写文案或添加一张图片', icon: 'none' })
        return
      }
      ...
      try {
        ...
        if (resp && resp.success === false) {
          const msg = resp.message || resp.msg || resp.error || '发布失败，请稍后重试'
          throw new Error(msg)
        }
        ...
        if (e && e.message) {
          errorMsg = e.message
          // 如果是网络错误，提供更友好的提示
          if (e.message.includes('timeout') || e.message.includes('网络')) {
            errorMsg = '网络连接失败，请检查网络后重试'
          } else if (e.message.includes('域名') || e.message.includes('合法域名')) {
            errorMsg = '请在小程序后台配置 uploadFile 合法域名：api.smallpeppers.cn'
          } else if (e.message.includes('401') || e.message.includes('未授权')) {
            errorMsg = '登录已过期，请重新登录'
          } else if (e.message.includes('500') || e.message.includes('服务器')) {
    ...
    async loadGlobalCheckins() {
      try {
        const cityName = this.cityInfo?.name || ''

        // 只按当前城市筛选，避免不同城市的打卡互相混在一起
        const res = await getAllCheckins({ page: 1, size: 100, city: cityName })
        let list = []
        ...
        this.posts = (list || []).map((item) => {
          ...
          const serverId = item.id || item.checkinId || item.recordId || item.checkInId || null
          const createdAt = item.createdAt || item.createTime || item.timestamp || Date.now()
          ...
          return {
            ...
            city: item.city || this.cityInfo?.name || ''
          }
        })

        this.savePostsToStorage()
      } catch (e) {
        console.warn('[noodle-detail] 加载全局打卡失败，使用本地兜底:', e)
    ...
    removePost(idx) {
      const item = this.posts[idx]
      if (!item) {
        uni.showToast({ title: '打卡记录不存在', icon: 'none' })
        return
      }
      ...
      uni.showModal({
        ...
        success: async (res) => {
          ...
          const serverId = item.checkinId || item.id || item.checkInId || item.recordId
          const isLocalOnly = !serverId
          ...
          try {
            ...
            console.error('[removePost] 删除打卡失败:', e)
    ...
    goMustEat() {
      const query = this.cityId ? `?city=${this.cityId}` : ''
      uni.navigateTo({ url: `/pages/noodle/must-eat${query}` })
    },
    openPostDialog() {
    ...
    locateWithAMap() {
      ...
      uni.getLocation({
        ...
        success: (pos) => {
          ...
          // 使用高德逆地理编码获取地址
          try {
            const location = `${pos.longitude},${pos.latitude}`
            myAmapFun.getRegeo({
              location: location,
              success: (data) => {
                const item = Array.isArray(data) ? data[0] : data
                const addr = item?.name || item?.desc || (item?.regeocodeData && item.regeocodeData.formatted_address) || ''
                this.draftLocation = addr || location || this.cityInfo?.name || ''
                uni.showToast({ title: '定位成功', icon: 'success' })
              },
              fail: (err) => {
                console.warn('AMap getRegeo fail:', err)
                // 即使逆地理编码失败，也保留坐标和城市名作为位置
                this.draftLocation = this.cityInfo?.name || `${pos.latitude},${pos.longitude}`
                uni.showToast({ 
                  title: `定位成功，但获取地址失败：${err.errMsg || '请检查高德Key配置'}`,
                  icon: 'none',
                  duration: 3000
                })
              }
...
.section-title--with-action {
  ...
  align-items: center;
  justify-content: space-between;
  margin: 24rpx 24rpx 12rpx 24rpx; /* 上方间距24rpx，右侧24rpx，下方12rpx，左侧40rpx（往右移动） */
}
.section-action {
  background: #ffd98a;
  color: #7b2f00;
